...
| 🗺️ **Spatial** | Plot cells at their tissue x/y coordinates, colored by any metadata variable (categorical → lineage palette, numeric → viridis), with a **sample** picker to focus on / facet specific tissue sections. Pick which loaded dataset to plot from the sidebar. Works on objects that keep coordinates in meta.data (x/y) or in @images. |
