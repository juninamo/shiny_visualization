...
| 🔥 **Heatmap / Dot** | One tab with shared gene/cluster controls and two inner sub-tabs: a **Heatmap** (mean-expression, optional per-gene Z-scoring and row/column clustering) and a **Dot Plot** (dot size = % expressing, color = scaled mean, optional marker-group facets). With a reference, both plots have a **Combine** mode; the Dot Plot's combine also outputs a **correspondence table** (each active cluster → nearest reference by expression correlation, with a confidence *margin*) that you can **download as CSV**. |
