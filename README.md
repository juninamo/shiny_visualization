...
| 🗺️ **Spatial** | Plot cells at their tissue x/y coordinates, colored by any metadata variable (categorical → lineage palette, numeric → viridis) **or by a single gene's expression** (viridis), with a **sample** picker to focus on / facet specific tissue sections. If the object has **segmentation** (cell-boundary polygons in @images), a toggle draws filled cell shapes instead of points. You can **highlight selected clusters** (others greyed) and, in a **Neighborhood** sub-tab, plot the nearest-neighbor distance ECDF from the selected clusters to every other cluster (x = distance, log scale; y = cumulative fraction) — closer clusters' curves sit further left; hover a line to see which cluster. Two more sub-tabs reproduce squidpy-style analyses: **Co-occurrence** (P(j \| i,d)/P(j) vs distance) and **Neighbors enrichment** (permutation z-score matrix on a spatial kNN graph, with a **symmetric** mode like squidpy's `nhood_enrichment` — undirected edge counts, so z(A,B)=z(B,A) — and a **directed** mode that counts how often column j appears in the k-neighborhood of center cluster i, giving an asymmetric z(i→j)≠z(j→i) that distinguishes "B around A" from "A around B"). For the Neighborhood and Co-occurrence curves the across-sample band is **selectable** — **95% CI of the mean** (default, = 1.96·SE), **±1 SE**, **±1 SD** (raw between-sample spread), or **none** — so it isn't dominated by sample-to-sample variance. Pick which loaded dataset to plot from the sidebar. Works on objects that keep coordinates in meta.data (x/y) or in @images. For **very large spatial objects** (e.g. millions of cells), the tab has its **own loader** so the object is loaded *only into this tab* and never handed to the other tabs — this keeps the rest of the app responsive. Such objects render **one sample at a time** (drawing all samples at once is blocked with a prompt to pick a sample), and metadata columns are classified once at load (not on every redraw). Each sub-tab also has a collapsible **illustrated "how it works"** diagram explaining the analysis. |
...
| 🧩 **Niche** | Visualize spatial **niches identified by [tessera](https://github.com/JEFworks-Lab/tessera)**. This tab loads its **own** files (the niche data are plain `data.frame`s, not Seurat objects): a **tile_meta** `.rds` (one `sf` polygon per niche tile + a niche-cluster column such as `seurat_clusters`) and, optionally, a per-cell **annotation** `.rds`. Pick a **sample** and a **niche variable**, and the tiles are drawn as filled polygons colored by niche (interactive: **hover a tile to see its niche**; with optional **niche highlighting** and Y-flip). If a cell-annotation file is loaded, you choose which **cell-type column** to use and get a **niche × cell-type composition** heatmap (each niche row-normalized) + **CSV** of what cell types make up each niche, for the displayed sample or across all samples. The cell→tile link is `tile_id`; if the annotation file lacks it (e.g. a merged fine-cell-type `meta.rds`), it is **auto-recovered from a sibling `clusters_meta`** by matching cell IDs, and per-sample filtering is done by tile membership (so a different `sample_id` naming in the annotation file is fine). Samples without cell-type annotation still draw the map and show a clear message for the composition. A **niche proximity enrichment** section applies the same permutation z-score Neighbors-enrichment as the Spatial tab (here on niche tile centroids + niche labels) to reveal **which niches sit next to which** — with the same symmetric / directed modes and significance asterisks. The map, composition and proximity sections each include a collapsible **illustrated "how it works"** diagram. |
| 🧮 **Punkst** | Visualize the cell-level output of **[Punkst](https://github.com/Yichen-Si/punkst)** (segmentation-free spatial factorization). This tab loads its own files: a **factor/cluster `.rds`** (per-cell `top_factor`, annotations, `sample_id`) and a **coordinate file** (a cell-meta/Seurat `.rds` with x/y, joined by rownames or `cell_id`). Cells are drawn at their x/y positions colored by `top_factor` / annotation, per sample (multi-sample faceting; large datasets render one sample at a time). The **topic detail files** (the Punkst `*_color.rgb.tsv` palette and `*.report.factor.info.tsv` / `*.de.tsv` marker-gene tables) can be **uploaded separately** — the color file sets the canonical factor palette and the info file shows each factor's top genes. A **topic × cell-type / niche association** heatmap cross-tabulates each cell's `top_factor` against a chosen annotation (cell type, or — by optionally loading a tessera tile file to recover a per-cell `niche` — the niche), with per-topic / per-variable / log2(observed÷expected) **enrichment** normalization + CSV. Finally, uploading the **`*.pseudobulk.tsv`** (genes × topics) enables **topic-vs-topic DEG** (CPM-normalized log2 fold change of Topic A vs B or all others) with a top-genes bar + table + CSV, and **GSEA** (`fgsea` × MSigDB) on that ranking. |
