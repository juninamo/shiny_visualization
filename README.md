...
| 🧭 **Azimuth** | Automated cell-type annotation with **Azimuth** (`RunAzimuth`) against a reference such as **Pan-Human Azimuth**. Point it at a downloaded reference folder (containing `idx.annoy` + `ref.Rds`) placed in the app directory; it maps the active dataset, then shows a UMAP of the predicted labels, a summary table (counts + mean prediction / mapping scores), and a CSV download of the per-cell annotations. |
...
# Azimuth (optional) for the Azimuth annotation tab:
